/**
 * Full DOCX Generator UMD Module
...
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  ...
  function generateParagraphXml(paragraph) {
    ...
    if (paragraph.heading) {
      switch (paragraph.heading) {
        case HeadingLevel.TITLE:
          styleXml = '<w:pStyle w:val="Title"/>';
          spacingXml = '<w:spacing w:after="300" w:before="0"/>';
          break;
        case HeadingLevel.HEADING_1:
          styleXml = '<w:pStyle w:val="Heading1"/>';
          spacingXml = '<w:spacing w:after="120" w:before="240"/>';
          break;
        case HeadingLevel.HEADING_2:
          styleXml = '<w:pStyle w:val="Heading2"/>';
          spacingXml = '<w:spacing w:after="100" w:before="200"/>';
          break;
        case HeadingLevel.HEADING_3:
          styleXml = '<w:pStyle w:val="Heading3"/>';
          spacingXml = '<w:spacing w:after="80" w:before="160"/>';
          break;
      }
    } else {
      // Regular paragraph spacing
      spacingXml = '<w:spacing w:after="120" w:before="0"/>';
    }
    ...
    lines.forEach((line, index) => {
      ...
      if (line.trim()) {
        runsXml += `<w:r><w:t xml:space="preserve">${line}</w:t></w:r>`;
      } else if (lines.length > 1) {
        // Empty line but not the only line
        runsXml += '<w:r><w:t> </w:t></w:r>';
      }
    });
